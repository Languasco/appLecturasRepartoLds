...
namespace DSIGE.Web.Controllers
{
    public class Importar_ExcelController : Controller
    {
        ...
        [HttpPost]
        public string GrabarExcel_cortesReconexiones( string fechaEnvioMovil, int idServicio, int idOpcion)
        {
            object loDatos = null;
            try
            {
                ...
                if (idOpcion == 1)
                {
                    loDatos = Objeto_Negocio.Capa_Negocio_save_CargaProgramacion(fechaEnvioMovil, idServicio, idOpcion, usuario);

                }
                else if (idOpcion == 2)
                {
                    loDatos = Objeto_Negocio.Capa_Negocio_save_MacroOrdenes(fechaEnvioMovil, idServicio, idOpcion, usuario);
                }
                else if (idOpcion == 3)
                {
                    loDatos = Objeto_Negocio.Capa_Negocio_save_MacroOperaciones(fechaEnvioMovil, idServicio, idOpcion, usuario);
                }
                return _Serialize(loDatos, true);
            }
            catch (Exception ex)
        ...
        public string InsertaExcel_lds(HttpPostedFileBase file)
        {
            ...
            try
            {
                ...
                Cls_Negocio_Importacion_Lecturas Objeto_Negocio = new Cls_Negocio_Importacion_Lecturas();
                loDatos = Objeto_Negocio.Capa_Negocio_save_temporalImportarLds(fileLocation, ((Sesion)Session["Session_Usuario_Acceso"]).usuario.usu_id, NombreArchivo );
        ...
        [HttpPost]
        public string GrabarExcel_importarLds(string fechaRecepcion, string fechaInicio, string fechaTermino, int tipoReparto,  int anio, int mes)
        {
            object loDatos = null;
            try
            {
                ...
                loDatos = Objeto_Negocio.Capa_Negocio_save_importacionLds(fechaRecepcion, fechaInicio, fechaTermino, usuario, tipoReparto, anio,  mes);
